...
from app.services.rag_ext.fusion.resultFusion import fuse_results
from app.services.rag_ext.rerank.reranker import heuristic_rerank
from app.services.rag_ext.retrieval.obsidianRetriever import retrieve_obsidian
...
async def stream_reply(conversation_id: int, payload: MessageIn, db: Session = Depends(get_db), user: User = Depends(get_actor_user)):
    ...
    if payload.use_rag:
        ...
        try:
            ...
            pdf_hits = []
            if payload.collection_ids or payload.metadata_filters:
                ...
                for h in pdf_hits:
                    h["source"] = h.get("source") or "pdf"
                    h["score"] = float(h.get("score", 0.5) or 0.5)

            obs_hits = []
            if payload.use_obsidian:
                obs_hits = await retrieve_obsidian(
                    payload.content,
                    top_k=max(6, target_k // 2),
                    metadata_filters=payload.obsidian_filters,
                    prefer_obsidian=payload.prefer_obsidian,
                )

            merged = fuse_results(pdf_hits, [], obs_hits, top_k=target_k, prefer_obsidian=payload.prefer_obsidian)
            reranked = heuristic_rerank(payload.content, merged, top_k=target_k)
            compressed = compress_context(reranked, max_chars=1800)
            citations = [
                {
                    ...
                    "file_path": h.get("file_path") or h.get("filePath"),
                    "heading": h.get("heading"),
                    "score": h.get("fusion_score") or h.get("rerank_score") or h.get("score"),
            ...
            if citations:
                context = "\n\nSources récupérées (PDF/Obsidian):\n" + "\n".join(
                    [
                        f"- [{c.get('source','pdf').upper()}] [doc:{c['doc_id']} p.{c['page']}] {c['title']}"
                        + (f" | {c.get('heading')}" if c.get('heading') else "")
                        + (f" | {c.get('file_path')}" if c.get('file_path') else "")
                        + f": {c['excerpt']}"
                        for c in citations
                    ]
                )
                    ...
                    with SessionLocal() as writer_db:
                        ai_msg = Message(
                            ...
                            metadata_json={
                                ...
                                "used_obsidian": bool(payload.use_obsidian),
                                "prefer_obsidian": bool(payload.prefer_obsidian),
                            },
                        )
