...
## 12) Intégration Obsidian Vault (Markdown)
- Source additionnelle RAG: notes Obsidian `.md` via:
  - mode filesystem (chemin du vault)
  - mode Obsidian Local REST API (base URL + API key).
- Indexation incrémentale: hash fichier + mtime (`data/obsidian_manifest.json`) pour éviter un re-embed complet.
- Filtres metadata Obsidian: tags, noteType, course, language, fenêtre de récence.
- Transparence UI: citations avec source `PDF` vs `OBSIDIAN`, drawer des sources et bouton d’ouverture de note (`obsidian://open`).

Voir `docs/OBSIDIAN_RAG_INTEGRATION.md` pour les détails techniques.
